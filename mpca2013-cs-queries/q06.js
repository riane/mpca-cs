...
MongoClient.connect(DatabaseUrl, function(err, db) {
  ...
  collection.find({},{_id:0, 'municipio.nome':1}).each(function(err, item) {
	if(item != null) console.dir(item.municipio.nome);
  });

  // Retorna a quantidade de municipios
  collection.find({},{_id:0, 'municipio.nome':1}).toArray(function(err, items) {          
    console.log(" ");
    console.log(">> Quantidade de municipios: " + items.length);
